...
interface PropertyAssignmentFieldsProps {
  routeGroupId: string
...
/**
 * The property-assignment body embedded in the Route Group sheet (alongside
 * the name form) — not a Sheet of its own. Properties are grouped by account
 * for scanability, but every property gets its own toggle — no account-level
 * bulk control. A property already assigned to a DIFFERENT route group can
 * still be toggled here; flipping it prompts a confirmation dialog (moving it
 * out of its current group) rather than being disabled outright.
 */
export function PropertyAssignmentFields({
  routeGroupId,
  routeGroupName,
  allProperties,
}: PropertyAssignmentFieldsProps) {
  ...
  return (
    <>
      ...
      {reassignTarget && (
        // Rendered inline (not a separate Radix Dialog) deliberately — nesting a
        // second Radix Dialog/Sheet root inside the parent Route Group Sheet
        // causes their dismissable-layer stacks to cross-dismiss each other on
        // outside clicks (well-documented Radix issue), closing the whole sheet
        // whenever the confirmation is dismissed. A plain absolutely-positioned
        // overlay sidesteps that entirely — it just needs the parent SheetContent
        // to be the nearest positioned ancestor, regardless of nesting depth.
