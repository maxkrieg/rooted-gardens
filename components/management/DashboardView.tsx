...
export function DashboardView() {
  ...
  // Rows carry property + account already, so this reassembles exactly the shape
  // the RSC version fetched with its own embedded query.
  const visits: VisitWithDetails[] = useMemo(() => {
    const week = weeks[0]
    if (!week) return []
    return [...week.routeGroups.flatMap((g) => g.rows), ...week.ungrouped]
      .filter((row) => row.visit)
      .map((row) => ({ ...row.visit!, property: row.property, account: row.account }) as VisitWithDetails)
  }, [weeks])

  if (!hydrated || (isLoading && !hasData)) return <DashboardSkeleton />

  // Fail per section, not per page. A failure must not fall through to `[]`:
  // that renders "Nothing scheduled this week" during an outage, which is untrue.
  const visitsFailed = isError && !hasData
  const fleetFailed = fleet.isError && !fleet.data
  ...
  const uninvoicedCount = visits.filter((v) => v.status === 'completed' && !v.invoice_id).length
  ...
  const maintenanceEquipment = fleet.data?.equipment ?? []
  const maintenanceVehicles = fleet.data?.vehicles ?? []
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      ...
      {/* Only when something needs attention — but a failure still renders:
          hiding it would imply nothing needs service. */}
    ...
    <div className="rounded-xl border border-border bg-card px-4 py-3 shadow-warm space-y-2">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground mt-0.5 truncate">{visit.account.name}</p>
        </div>
  ...
  return (
    <div className="rounded-xl border border-[var(--clay)]/30 bg-[var(--clay)]/[0.05] px-4 py-3 space-y-1.5">
      <div className="flex items-center gap-1.5">
        ...
        <p className="text-xs font-semibold text-[var(--clay)] truncate">{visit.property.address}</p>
      </div>
